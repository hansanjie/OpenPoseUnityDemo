...
namespace TriLib
{
    ...
    /// <summary>
    /// Abstract representation of an asset loader.
    /// </summary>
    public abstract class AssetLoaderBase : IDisposable
    {
        ...
        protected void InternalLoadFromMemory(byte[] fileBytes, string filename, AssetLoaderOptions options = null)
        {
            IntPtr scene;
            try
            {
        ...
        /// <param name="options"><see cref="TriLib.AssetLoaderOptions"/> used to load the file.</param>
        /// <returns></returns>
        protected void InternalLoadFromMemoryAndZip(byte[] data, string assetExtension, AssetLoaderOptions options = null)
        {
#if TRILIB_USE_ZIP
#if !UNITY_EDITOR && (NET_4_6 || NETFX_CORE)
...
#endif
#endif
                        if (zipEntry.Name.ToLowerInvariant() == fileShortName)
                        {
#if TRILIB_USE_ZIP
#if !UNITY_EDITOR && (NET_4_6 || NETFX_CORE)
                            var zipStream = zipEntry.Open();
            ...
            InternalLoadFromMemory(data, assetExtension, options);
        }
        ...
        /// <param name="options"><see cref="TriLib.AssetLoaderOptions" /> used to load the file.</param>
        protected void InternalLoadFromFile(string filename, AssetLoaderOptions options = null)
        {
            IntPtr scene;
            try
            {
        ...
        protected GameObject BuildGameObject(AssetLoaderOptions options, string basePath = null, GameObject wrapperGameObject = null)
        {
            ...
            if (gameObject != null)
            {
                ...
                {
                    SetupAnimations(gameObject, options);
                }
        ...
        {
            var dummy = false;
            var hasAlphaChannelOnTextures = options != null && options.ApplyAlphaMaterials;

            var diffuseTexture = materialData.DiffuseInfoLoaded ? LoadTextureFromFile(materialData.DiffusePath, materialData.Name, options, materialData.DiffuseEmbeddedTextureData, materialData.DiffuseWrapMode, ref hasAlphaChannelOnTextures, false, basePath) : null;
            ...
            var hasAlpha = materialData.AlphaLoaded && materialData.Alpha != 1f || hasAlphaChannelOnTextures;
        ...
        /// <returns></returns>
        protected virtual Texture2D LoadTextureFromFile(string path, string name, AssetLoaderOptions options, EmbeddedTextureData embeddedTextureData, TextureWrapMode textureWrapMode, ref bool checkAlphaChannel, bool isNormalMap, string basePath)
        {
            Texture2D texture;
            ...
            {
                texture = Texture2DUtils.LoadTextureFromFile(path,
                    embeddedTextureData != null ? embeddedTextureData.Width : 0,
                    embeddedTextureData != null ? embeddedTextureData.Height : 0, name,
                    ref checkAlphaChannel, embeddedTextureData != null ? embeddedTextureData.Data : null,
                    embeddedTextureData != null && embeddedTextureData.IsRawData, isNormalMap,
                    basePath, textureWrapMode, options != null ? options.TextureCompression : TextureCompression.NormalQuality);
                if (texture != null)
                {
                    texture.name = name;
        ...
        /// <param name="options"><see cref="AssetLoaderOptions"/> used to load the object.</param>
        private void LoadInternal(IntPtr scene, AssetLoaderOptions options)
        {
            Dispose();
        ...
        private void BuildObjects(IntPtr scene, AssetLoaderOptions options)
        {
            NodesPath = new Dictionary<string, string>();
            var rootNode = AssimpInterop.aiScene_GetRootNode(scene);
            RootNodeData = BuildObject(RootNodeData, rootNode, options);
        }
        ...
        private NodeData BuildObject(NodeData parentNodeData, IntPtr node, AssetLoaderOptions options)
        {
            var nodeId = NodeId++;
            ...
            var nodePath = parentNodeData != null ? string.Format("{0}/{1}", parentNodeData.Path, nodeName) : string.Format("/{0}", nodeName);
            ...
            if (childrenCount > 0)
            {
                ...
                for (uint c = 0; c < childrenCount; c++)
                {
                    ...
                    var childNodeData = BuildObject(nodeData, childNode, options);
        ...
        private void BuildMetadata(IntPtr scene)
        {
            ...
            for (uint i = 0; i < metadataCount; i++)
            {
                ...
                Metadata[metadataCount] = new AssimpMetadata(metadataType, i, metadataKey, metadataValue);
            }
        }
        ...
        /// </summary>
        /// <param name="scene">Assimp scene pointer.</param>
        private void BuildMaterials(IntPtr scene)
        {
                ...
                if (numDiffuse > 0)
                {
                        ...
                        out blendMode, out op, out mapMode))
                    {
                        ...
                        if (embeddedTextureData == null)
                        {
                            ...
                            {
                                embeddedTextureData = BuildEmbeddedTextureData(scene, texture);
                            }
                        }
        ...
        /// <summary>
        /// Builds and appends the <see cref="TriLib.EmbeddedTextureData"/> to the list.
        /// </summary>
        /// <param name="scene">Assimp scene pointer.</param>
        /// <param name="texture">Assimp texture pointer.</param>
        private EmbeddedTextureData BuildEmbeddedTextureData(IntPtr scene, IntPtr texture)
        {
